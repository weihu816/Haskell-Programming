...
Announcements
-------------
* Lectures based on this file plus [Client.html](Client.html).
* Exercise: [DList](DList.html)
* Exercise: [TransC](TransC.html)


* SIGN up for demo slots (See piazza)
* We will start with DList exercise today
...
~~~~~~{.haskell}
    ...
    Main*> sched [ Fork (hello552Computation Stop) (hello552Computation Stop) ]
    Main*> let bomb = Fork (writeComputation "bomb" bomb) Stop
    Main*> let bomb = Fork (writeComputation "bomb" bomb) bomb
~~~~~~
...
Let's abbreviate the type, to make it easier to write:
...
Let's define some operations that can help us build computations
in the `C` monad.
