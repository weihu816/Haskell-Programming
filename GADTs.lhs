...
>     _                                -> Nothing
> oevaluate (OIsZero e1)   = case oevaluate e1 of
>	Just (Left x) -> if x == 0 then Just (Right True) else Just (Right False)
>	_ -> Nothing
> oevaluate (OIf e1 e2 e3) = case oevaluate e1 of
>	Just (Right x) -> if x then oevaluate e3 else oevaluate e2
>	_ -> Nothing
