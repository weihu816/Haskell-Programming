...
* TODAY: type inference!
   ...
   Need files:
    (old) [Parsers](Parsers.hs), [FunSyntax](FunSyntax.hs), [FunEnv](FunEnv.hs),
          [FunEnvError](FunEnvError.hs),
    (new) [FunTypes](FunTypes.hs) (this file)
...
For warmup, let's review error reporting to the environment based interpreter.
...
to form a single substitution that applies the substitutions in `s1`
*after* it applies those in `s2`.  This composition is only valid
