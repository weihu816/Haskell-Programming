Introduction to Monads (and Applicative Functors!)
==================================================
...
* Find your new table and partner
* HW #6 available   (due Monday after Spring break)
* Download this file (from lecture schedule)
* Start working on the exercise below
...
(Note: we're going to look at several different versions of this safe zipTree
function, so, we'll parameterize the test case by the version that we are
testing.)
...
Given this Applicative functor instance, perhaps we could have derived a
slightly different set of generic operators in the zipTree example...
Can we rewrite with Applicatives?
...
=============
* back to Monads.lhs lecture
* start working on pairs0 challenge below
* poll about HW 6 due date
...
(Note: guard is defined for *some* monads)
...
Can we rewrite some of these examples using Applicative instance instead?
